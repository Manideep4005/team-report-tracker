...
export default function Sidebar({
  ...
  /*
   * Separate refs for mobile
   * and desktop account cards.
   */
  const accountRefDesktop =
    useRef<HTMLDivElement>(null);

  const accountRefMobile =
    useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    function handleOutsideClick(
      ...
      const insideDesktop =
        accountRefDesktop.current?.contains(
          target
        );


      const insideMobile =
        accountRefMobile.current?.contains(
          target
        );
   ...
   * IMPORTANT:
   *
   * The section itself is only shown
   * when at least one permitted item
   * exists.
   */
  const showManagementSection =
    ...
    hasPermission("PUBLIC_MONITOR_VIEW");
